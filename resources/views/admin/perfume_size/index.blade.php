    ...
    <div class="row breadcrumbs-top">
     <div class="breadcrumb-wrapper col-12">
      <ol class="breadcrumb">
       ...
       <li class="breadcrumb-item"><a href="{{ route('admin.sizes.create') }}">Create Perfume Size</a>
       </li>
       <li class="breadcrumb-item active">Perfume Size Responsive Datatable
       </li>
      </ol>
     </div>
    </div>
      ...
      <table class="table table-striped table-bordered dataex-res-configuration">
       ...
       <tbody>
        ...
        <tr>
         <td>{{ ++$key }}</td>
         <td>{{ $size->name }}</td>
         <td>{{ $size->created_at->format('F j, Y') }}</td>
         <td>{{ $size->updated_at->format('F j, Y') }}</td>
         <td>
          <a href="{{ route('admin.sizes.edit', $size->id) }}" class="btn btn-warning btn-sm">Edit</a>
          <a href="{{ route('admin.sizes.show', $size->id) }}" class="btn btn-primary btn-sm">Show</a>
