    ...
    <div class="row breadcrumbs-top">
     <div class="breadcrumb-wrapper col-12">
      <ol class="breadcrumb">
       ...
       <li class="breadcrumb-item"><a href="{{ route('admin.scents.create') }}">Create Perfume Scent</a>
       </li>
       <li class="breadcrumb-item active">Perfume Scent Responsive Datatable
       </li>
      </ol>
     </div>
    </div>
     ...
     <div class="card-body card-dashboard">
      ...
      <table class="table table-striped table-bordered dataex-res-configuration">
       ...
       <tbody>
        ...
        <tr>
         <td>{{ ++$key }}</td>
         <td>{{ $scent->scent_name }}</td>
         <td>{{ $scent->created_at->format('F j, Y') }}</td>
         <td>{{ $scent->updated_at->format('F j, Y') }}</td>
         <td>
          <a href="{{ route('admin.scents.edit', $scent->id) }}" class="btn btn-warning btn-sm">Edit</a>
          <a href="{{ route('admin.scents.show', $scent->id) }}" class="btn btn-primary btn-sm">Show</a>
          <form class="d-inline" action="{{ route('admin.scents.destroy', $scent->id) }}" method="POST">
           @csrf
           @method('DELETE')
