    ...
    <div class="row breadcrumbs-top">
     <div class="breadcrumb-wrapper col-12">
      <ol class="breadcrumb">
       ...
       <li class="breadcrumb-item"><a href="{{ route('admin.product_brands.create') }}">Create Brand</a>
       </li>
       <li class="breadcrumb-item active">Brand Responsive Datatable
       </li>
      </ol>
     </div>
    </div>
      ...
      <table class="table table-striped table-bordered dataex-res-configuration">
       ...
       <tbody>
        ...
        <tr>
         ...
         <td><img src="{{ asset('uploads/' . $brand->logo) }}" width="50px" alt="{{ $brand->name }} Logo">
         </td>
         <td>{{ $brand->created_at->format('F j, Y') }}</td>
         <td>{{ $brand->updated_at->format('F j, Y') }}</td>
         <td>
          <a href="{{ route('admin.product_brands.edit', $brand->id) }}" class="btn btn-warning btn-sm">Edit</a>
          <a href="{{ route('admin.product_brands.show', $brand->id) }}" class="btn btn-primary btn-sm">Show</a>
          <form class="d-inline" action="{{ route('admin.product_brands.destroy', $brand->id) }}" method="POST">
           @csrf
           @method('DELETE')
